...
def unie(mi, emin, emax): # uniformlize the exponents
    mo = np.array(mi, dtype = np.float32)
    for i0 in list(range(len(mi))):
        for i1 in list(range(len(mi[0]))):
            mo[i0][i1] = mi[i0][i1]*(10**(np.random.randint(emin, emax+1)))
    return mo
...
np.random.seed(315)
emin = -1 # min exponents number as [10^emin, 10^emax]
emax = 1  # max exponents number as [10^emin, 10^emax]
