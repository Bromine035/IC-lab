...
reg [2:0] cst, nst;
reg [31:0] r0, r1, r2, rru, rrw, rrv, rrh, rrx, rro, roval;
reg [31:0] ru [2:0][2:0];
reg [31:0] rw [2:0][2:0];
reg [31:0] rv [2:0][2:0];
reg [31:0] rh  [2:0];
...
wire [31:0] ma0z, md0z, me0z, mr0z;

assign wrnd = 3'b0;
assign wone = 32'b00111111100000000000000000000000;
assign wzpo = 32'b00111101110011001100110011001101;
assign wm0a = (cst == calc)?((r1 == 2)?(rv[r0][0]):((r1 == 1)?(rw[r0][0]):(ru[r0][0]))):(ma0z);
assign wm1a = (cst == calc)?((r1 == 2)?(rv[r0][1]):((r1 == 1)?(rw[r0][1]):(ru[r0][1]))):(32'b0);
assign wm2a = (cst == calc)?((r1 == 2)?(rv[r0][2]):((r1 == 1)?(rw[r0][2]):(ru[r0][2]))):(32'b0);
assign wm0b = (cst == calc)?((r1 == 0)?(rx[r2][0]):(rh[0])):(wzpo);
assign wm1b = (cst == calc)?((r1 == 0)?(rx[r2][1]):(rh[1])):(32'b0);
assign wm2b = (cst == calc)?((r1 == 0)?(rx[r2][2]):(rh[2])):(32'b0);
assign wea[30:0] = rvh[30:0];
assign wea[31] = ~(rvh[31]);
assign waa = (r1 == 2)?(wone):(rux[r0]);
assign wab = (r1 == 2)?(me0z):(rwh);
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n) begin
		r0 <= 32'b0; // 00 01 02 -> 10 11 12 -> 20 21 22
		r1 <= 32'b0; // ux -> wh -> vh
		r2 <= 32'b0; // x1 y1 -> x2 y2 -> x3 y3
		rro <= 32'b0;
		roval <= 1'b0;
		ry[0][0] <= 32'b0;
		ry[0][1] <= 32'b0;
	...
	else if(cst == acti) begin
		r0 <= (r0 == 32'd2)?32'b0:(r0 + 1);
		r1 <= (r1 == 32'd2 && r0 == 32'd2)?32'b0:((r0 == 32'd2)?(r1+1):r1);
		r2 <= (r1 == 32'd2 && r0 == 32'd2)?(r2+1):r2;
		if(r1 == 1) begin
			if(r0 == 2) begin
				rh[0] <= rhh[0];
				rh[1] <= rhh[1];
				rh[2] <= (ma0z[31])?(md0z):(ma0z);
			end
			else begin
				rhh[r0] <= (ma0z[31])?(md0z):(ma0z);
			end
		end
		else if(r1 == 2) begin
			ry[r2][r0] <= mr0z;
