...
def tryr(id0: dict, id1: dict, ivr: np.ndarray, inow: int, patnum: int):
    d0 = id0.copy()
    d1 = id1.copy()
    vr = np.copy(ivr)
    pc = inow
    ...
    while(True):
        ...
        if(op == "000" and fu == '1'):
            ...
            if((vr[ns] + ni)*2 + 0x1000 < 0x1000 or (vr[ns] + ni)*2 + 0x1000 > 0x1ffe):
                return True, d0, vr, -1
    ...
    for i in range(kAddrMin+32, kAddrMax, 2):
        ...
        if func == 'ADD':
            ...
            if(i >= nex):
                ...
                ok0, od0, ovr, onex = tryr(d0, d1, vr, i, patnum)
                if(onex == -1):
