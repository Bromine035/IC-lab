...
reg [7:0] rv0 [3:0];
wire clk0, clk1, clk2, clk3, clk4, clk5, clk6, clk7, clk8;

//==============================================//
//                 GATED_OR                     //
//==============================================//

GATED_OR GATED_mgo0(.CLOCK(clk), .SLEEP_CTRL(!(cst == idle || cst == indt) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk0)); // indt
GATED_OR GATED_mgo1(.CLOCK(clk), .SLEEP_CTRL(!(cst == con0 || cst == con1) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk1)); // conv
GATED_OR GATED_mgo2(.CLOCK(clk), .SLEEP_CTRL(!(cst == qua0 || cst == qua1) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk2)); // quat
GATED_OR GATED_mgo3(.CLOCK(clk), .SLEEP_CTRL(!(cst == mxp0 || cst == mxp1) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk3)); // mxpl
GATED_OR GATED_mgo4(.CLOCK(clk), .SLEEP_CTRL(!(cst == den0 || cst == den1) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk4)); // dens
GATED_OR GATED_mgo5(.CLOCK(clk), .SLEEP_CTRL(!(cst == qud0 || cst == qud1) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk5)); // quad
GATED_OR GATED_mgo6(.CLOCK(clk), .SLEEP_CTRL(!(cst == lds0) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk6)); // ldis
GATED_OR GATED_mgo7(.CLOCK(clk), .SLEEP_CTRL(!(cst == act0) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk7)); // acti
GATED_OR GATED_mgo8(.CLOCK(clk), .SLEEP_CTRL(!(cst == oudt || cst == idle) && cg_en), .RST_N(rst_n), .CLOCK_GATED(clk8)); // oudt

//==============================================//
//                  design                      //
//==============================================//

conv mcon(.clk(clk1), .rst_n(rst_n), .img(), .ker(), .wout());
quat mqua(.clk(clk2), .rst_n(rst_n), .map(), .wout());
mxpl mmxp(.clk(clk3), .rst_n(rst_n), .map(), .wout());
dens mden(.clk(clk4), .rst_n(rst_n), .map(), .wet(), .wout());
quad mqud(.clk(clk5), .rst_n(rst_n), .vec(), .wout());
ldis mlds(.clk(clk6), .rst_n(rst_n), .vec0(), .vec1(), .rout());
acti mact(.clk(clk7), .rst_n(rst_n), .rin(mlds.rout), .rout());
generate
